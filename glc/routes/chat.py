...
async def _resolve_image_urls(messages):
    ...
    from glc.security.egress import EgressBlocked, safe_get
    ...
    async def _fetch_to_data_url(url: str) -> str:
        ...
        # C1: SSRF-safe fetch — every redirect hop is re-checked against the
        # private/loopback/link-local denylist, so a caller cannot steer the
        # gateway at cloud metadata or an internal service.
        try:
            r = await safe_get(url, timeout=30, headers=headers)
        except EgressBlocked as e:
            raise HTTPException(400, f"image url rejected: {e}") from e
        except _httpx.HTTPError:
            # Generic message to the client; specifics stay server-side.
            raise HTTPException(400, "failed to fetch image url") from None
...
# C4: the client sees a generic message; the specific provider, endpoint, and
# upstream error text stay server-side (already captured by db.log_call).
_GENERIC_UPSTREAM_ERROR = "upstream provider error; see gateway logs for detail"
_GENERIC_UNAVAILABLE = "all providers are currently unavailable; retry later"
...
async def chat(req: ChatRequest, request: Request):
    ...
    for _ in range(len(candidates) + 1):
        ...
        try:
            ...
            if explicit_override or not getattr(e, "retryable", True):
                raise HTTPException(502, _GENERIC_UPSTREAM_ERROR)
    ...
    raise HTTPException(503, _GENERIC_UNAVAILABLE)
